...
export default function Signup() {
	...
	const handleSubmit = () => {
		axios
			.post(
				`${process.env.REACT_APP_SERVER_API_URL}/member/signup`,
				{
	...
	const login = () => {
		axios.post(`${process.env.REACT_APP_SERVER_API_URL}/member/login`, {
			memberInfo,
		})
	}
		...
		<div className='flex-row align-center w-390 border-1'>
			...
			<div className='mb-4'>
				...
				<div className='flex space-x-2'>
					...
					<button
						className={`bg-sub text-[white] px-4 py-2 rounded-md ${
							servicePurpose === '근육량 증가'
								? 'bg-blue-500 text-white'
								: 'bg-gray-200 text-gray-700'
						}`}
						onClick={() =>
							handleServicePurposeChange('근육량 증가')
						}
					>
						근육량 증가
					</button>
