...
export default function SearchOption({
	...
	return (
		<div
			className={`flex m-5 rounded-md text-12 p-3 text-[white] ${
				searchOption.view ? 'bg-info_s' : 'bg-info'
			}`}
		>
			<button
				onClick={() => handleViewToggle(optionKey)}
				className={`${
					searchOption.view ? 'bg-main' : 'bg-sub'
				} rounded-lg w-55 m-3`}
			>
				{searchOption.title}
			</button>
			...
			<button
				...
				className={`${
					searchOption.view && searchOption.condition
						? 'bg-main'
						: 'bg-sub'
				} rounded-lg p-7 m-5`}
			>
